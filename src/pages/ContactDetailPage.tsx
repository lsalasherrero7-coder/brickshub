...
export default function ContactDetailPage() {
  ...
  const { data: suggestedProperties } = useSuggestedProperties(contact?.contact_type === "comprador" ? id : undefined);
  ...
  const [deleteNextActionOpen, setDeleteNextActionOpen] = useState(false);
  ...
  const handleDeleteNextAction = async () => {
    ...
    toast({ title: "Próxima acción eliminada" });
    setDeleteNextActionOpen(false);
  };
  ...
  const handleSaveNextAction = async () => {
    if (!actionType || !actionDate) {
      toast({ title: "Tipo y fecha son obligatorios", variant: "destructive" });
      return;
    }
  ...
  const handleAddInteraction = async () => {
    if (!intNotes.trim()) {
      toast({ title: "Escribe una nota", variant: "destructive" });
      return;
    }
  ...
  const hasNextAction = !!(contact.next_action_date || contact.next_action_type);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 flex-1">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center ${
              isBuyer ? "bg-info/10" : "bg-primary/10"
            }`}
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-medium">{portalLabel}</p>
              </div>
            </div>
            <div>
            ...
            {contact.property_id && (
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <Link
                    to={`/propiedades/${contact.property_id}`}
      ...
      {isBuyer && buyerProfile && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-display">Perfil de comprador</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 text-sm">
              {buyerProfile.property_type && (
                <div>
                  ...
                  <p className="font-medium">
                    {PROPERTY_TYPES.find((t) => t.value === buyerProfile.property_type)?.label || buyerProfile.property_type}
                  </p>
                </div>
              )}
              {buyerProfile.bedrooms_min && (
                <div>
                  ...
                  <p className="font-medium">{buyerProfile.bedrooms_min}+</p>
                </div>
              )}
              {buyerProfile.bathrooms_min && (
                <div>
                  <p className="text-xs text-muted-foreground">Baños</p>
                  <p className="font-medium">{buyerProfile.bathrooms_min}+</p>
                </div>
              )}
              {(buyerProfile.budget_min || buyerProfile.budget_max) && (
                <div>
                  ...
                  <p className="font-medium">
                    {buyerProfile.budget_min ? formatEuro(buyerProfile.budget_min) : "—"} -{" "}
                    {buyerProfile.budget_max ? formatEuro(buyerProfile.budget_max) : "—"}
                  </p>
                </div>
              )}
              {buyerProfile.garage && buyerProfile.garage !== "indiferente" && (
                <div>
                  <p className="text-xs text-muted-foreground">Garaje</p>
                  <p className="font-medium">{GARAGE_OPTIONS.find((o) => o.value === buyerProfile.garage)?.label}</p>
                </div>
              )}
              {buyerProfile.preferred_floor && buyerProfile.preferred_floor !== "indiferente" && (
                <div>
                  <p className="text-xs text-muted-foreground">Planta</p>
                  <p className="font-medium">{FLOOR_OPTIONS.find((o) => o.value === buyerProfile.preferred_floor)?.label}</p>
                </div>
              )}
              {buyerProfile.preferred_zones && buyerProfile.preferred_zones.length > 0 && (
                <div className="col-span-full">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {buyerProfile.preferred_zones.map((z: string) => (
      ...
      {/* Próxima Acción + Interacciones */}
      <div className={cn("grid grid-cols-1 gap-6", isVendedor ? "lg:grid-cols-2" : "lg:grid-cols-1")}>
        {/* Próxima Acción */}
        <Card className="border-primary/30">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-base flex items-center gap-2">
              <Clock className="w-5 h-5" /> Próxima Acción
            </CardTitle>
            {hasNextAction && (
              <Button
                type="button"
                ...
                onClick={() => setDeleteNextActionOpen(true)}
                title="Eliminar próxima acción"
              >
          ...
          <CardContent className="space-y-3">
            {hasNextAction && (
              <div className="p-3 rounded-lg bg-muted mb-3">
                <p className="font-medium text-sm">{(contact as any).next_action_type || "Acción"}</p>
                {(contact as any).next_action_date && (
                  <p className="text-xs text-muted-foreground">
                    {format(parseISO((contact as any).next_action_date), "dd/MM/yyyy HH:mm", { locale: es })}
                  </p>
                )}
            ...
            <div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn("w-full justify-start text-left", !actionDate && "text-muted-foreground")}
                  >
        ...
        {/* Historial de interacciones (solo vendedor) */}
        {isVendedor && (
          <Card>
            <CardHeader>
      ...
      <Tabs defaultValue={contact.property_id ? "visitas" : isBuyer ? "sugeridas" : "visitas"}>
        <TabsList>
          {contact.property_id && (
            <TabsTrigger value="visitas">
              <Clock className="w-4 h-4 mr-1" />Visitas
            </TabsTrigger>
          )}
          {isBuyer && (
            <TabsTrigger value="sugeridas">
              <Building2 className="w-4 h-4 mr-1" />Propiedades sugeridas
            </TabsTrigger>
          )}
        </TabsList>
        ...
        {contact.property_id && (
          <TabsContent value="visitas">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Visitas programadas</CardTitle>
              </CardHeader>
              <CardContent>
        ...
        {isBuyer && (
          <TabsContent value="sugeridas">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Propiedades sugeridas</CardTitle>
              </CardHeader>
              <CardContent>
      ...
      {/* Delete next action dialog */}
      <DeleteConfirmDialog
        open={deleteNextActionOpen}
        onOpenChange={setDeleteNextActionOpen}
        title="¿Eliminar próxima acción?"
        description="Esto eliminará la próxima acción del contacto y la quitará del calendario CRM y de Google Calendar."
        onConfirm={() => handleDeleteNextAction()}
      ...
      <DeleteConfirmDialog
        ...
        cascadeOptions={[{ key: "buyer_profile", label: "Perfil de comprador", defaultChecked: true }]}
