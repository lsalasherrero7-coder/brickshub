...
import { Search, Phone, MapPin, Globe, Plus, ShoppingCart, Home, Trash2, Pencil } from "lucide-react";
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((contact: any) => (
            <Card key={contact.id} className="hover:shadow-md transition-shadow h-full">
              <CardContent className="p-5 space-y-3">
                <div className="flex items-start justify-between">
                  <Link to={`/contactos/${contact.id}`} className="flex items-center gap-2 flex-1 min-w-0">
                    <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                      {contact.contact_type === "comprador" ? <ShoppingCart className="w-4 h-4 text-primary" /> : <Home className="w-4 h-4 text-primary" />}
                    </div>
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  <Badge variant="outline" className="text-xs">{contact.contact_type === "comprador" ? "Comprador" : "Vendedor"}</Badge>
